...
const source = "./favicons/logo.png"; // Source image(s). `string`, `buffer` or array of `string`
...
async function doFavicons() {
  try {
    const response = await favicons(source, configuration);
    const savePath = './dist/' + Date.now()
    await makeDir(savePath)
    response.images.forEach(item => {
      if (item.name.includes('favicon-')) {
        fs.writeFile(savePath + '/' + item.name, item.contents).then(() => {
          if (item.name.includes('48')) {
            toIco(item.contents).then(buf => {
              console.log('sss', buf)
              fs.writeFile(savePath + '/favicon.ico', buf)
            })
          }
        })
      }
